...
for image in camera.capture_continuous(capture, format='bgr', use_video_port=True):
    ...
    for c in contours:
	    ...
	    upload = im.upload_image(
	        'detected.jpg', title="detected and uploaded with pyimgur")

        #send image to whatsapp and sleep so messages don't get spammed
	    client = Client(os.environ.get('TWILIO_ACCOUNT_SID'),
	                    os.environ.get('TWILIO_AUTH_TOKEN'))
	    client.messages.create(from_=os.environ.get('TWILIO_PHONE_NUMBER'), to=os.environ.get(
	        'CELL_PHONE_NUMBER'), media_url=upload.link, body='Intruder alert!')
	    time.sleep(1)
